...
public final class DirectionalStrings {
    ...
    public static Block parseDirectional(Block context, String directioninfo)
            throws MondoMessage {
        ...
        for (int i = 0; i < directional.length; i += 2) {
            ...
            if (desired_face == null) {
                throw new MondoMessage(
                        String.format(
                                "Direction must be one of up/down/north/south/east/west, got '{0}'",
                                directional[i]));
            }
